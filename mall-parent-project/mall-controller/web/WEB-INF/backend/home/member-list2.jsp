...
<div class="x-body">
    ...
    <script>
        ...
        function customerCount(){
            $.ajax({
                url:"/backend/customer/login/count",
                type:"GET",
                async: false,//设置为同步
                contentType: "application/json",
                success:function(data){
                    $("#countSPAN").empty().text("共有数据：" + data.data + " 条");
                }
            })
        }
    </script>
    ...
    <script>
        ...
        function customerList(){
            $.ajax({
                url:"/backend/customer/login/list",
                type:"GET",
                async: false,//设置为同步
                contentType: "application/json",
                success:function(data){
                    ...
                    $.each(data.data,function(index, element){
                        ...
                        resultValue += "<td>" + element.login_account + "</td>";
                        resultValue += "<td>" + element.login_name + "</td>";
                        resultValue += "<td>" + element.activate_code + "</td>";
                        resultValue += "<td>" + element.creation_time + "</td>";
                        resultValue += "<td class=\"td-status\">";
                        var userStats = "";
                        if(element.userStats == 0){
                            resultValue += "<span class=\"layui-btn layui-btn-normal layui-btn-sm layui-btn-disabled\">已激活</span>";
                            userStats = "禁用";
                        }else{
                        ...
                        resultValue += "<i class=\"layui-icon\">&#xe642;</i>";
                        resultValue += "</a>";
                        resultValue += "<a onclick=\"x_admin_show('修改密码','member-password.html',600,400)\" title=\"修改密码\" href=\"javascript:;\">";
                        resultValue += "<i class=\"layui-icon\">&#xe631;</i>";
                        resultValue += "</a>";
